...
def plot_inner_domain(namelist: Namelist, turbine: Turbine, opt_params: Dict[str, Any]) -> plt.Figure:
    ...
    y_low_filtered  = y_pos_array[y_pos_array < turbine.turb_y]
    x_low_filtered  = x_pos_array[x_pos_array < turbine.turb_x]

    y_low  = y_low_filtered[-2] if len(y_low_filtered) > 1 else 0
    y_high = (y_pos_array[y_pos_array > turbine.turb_y])[1]
    ...
    rectangle = Rectangle(
        ...
        x_high - x_low,            # Width of the rectangle
        y_high - y_low,            # Height of the rectangle
